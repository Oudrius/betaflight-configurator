...
vtx.initialize = function (callback) {
    ...
    function read_vtx_config_json(vtxConfig, vtxcallback_after_read) {
        ...
        for (let i = 1; i <= FC.VTX_CONFIG.vtx_table_bands; i++) {
            ...
            TABS.vtx.VTXTABLE_BAND_LIST[i - 1].vtxtable_band_is_factory_band =
                vtxConfig.vtx_table.bands_list[i - 1].is_factory_band;
            TABS.vtx.VTXTABLE_BAND_LIST[i - 1].vtxtable_band_frequencies =
                vtxConfig.vtx_table.bands_list[i - 1].frequencies;
        ...
        for (let i = 1; i <= FC.VTX_CONFIG.vtx_table_powerlevels; i++) {
            ...
            TABS.vtx.VTXTABLE_POWERLEVEL_LIST[i - 1].vtxtable_powerlevel_value =
                vtxConfig.vtx_table.powerlevels_list[i - 1].value;
            TABS.vtx.VTXTABLE_POWERLEVEL_LIST[i - 1].vtxtable_powerlevel_label =
                vtxConfig.vtx_table.powerlevels_list[i - 1].label;
        }
    ...
    function initDisplay() {
        ...
        // Supported?
        const vtxSupported =
            FC.VTX_CONFIG.vtx_type !== VtxDeviceTypes.VTXDEV_UNSUPPORTED &&
            FC.VTX_CONFIG.vtx_type !== VtxDeviceTypes.VTXDEV_UNKNOWN;
        const vtxTableNotConfigured =
            vtxSupported &&
        ...
        const vtxReady_e = $(".VTX_info span.colorToggle");
        vtxReady_e.text(
            FC.VTX_CONFIG.vtx_device_ready ? i18n.getMessage("vtxReadyTrue") : i18n.getMessage("vtxReadyFalse"),
        );
        ...
        $("#vtx_low_power_disarm_description").text(
            i18n.getMessage(`vtxLowPowerDisarmOption_${FC.VTX_CONFIG.vtx_low_power_disarm}`),
        );
        ...
        if (FC.VTX_CONFIG.vtx_power === 0) {
            ...
            if (FC.VTX_CONFIG.vtx_table_available) {
                let powerLevel =
                    TABS.vtx.VTXTABLE_POWERLEVEL_LIST[FC.VTX_CONFIG.vtx_power - 1].vtxtable_powerlevel_label;
                if (powerLevel.trim() === "") {
        ...
        for (let i = 1; i <= TABS.vtx.VTXTABLE_BAND_LIST.length; i++) {
            ...
            $(`#vtx_table_band_factory_${i}`).prop(
                "checked",
                TABS.vtx.VTXTABLE_BAND_LIST[i - 1].vtxtable_band_is_factory_band,
            );
            ...
            for (let j = 1; j <= TABS.vtx.VTXTABLE_BAND_LIST[i - 1].vtxtable_band_frequencies.length; j++) {
                $(`#vtx_table_band_channel_${i}_${j}`).val(
                    TABS.vtx.VTXTABLE_BAND_LIST[i - 1].vtxtable_band_frequencies[j - 1],
                );
            }
        }
        ...
        $("#vtx_frequency_channel")
            .prop("checked", FC.VTX_CONFIG.vtx_band === 0 && FC.VTX_CONFIG.vtx_frequency > 0)
        ...
        function populateChannelSelect() {
            ...
            if (FC.VTX_CONFIG.vtx_table_available) {
                if (TABS.vtx.VTXTABLE_BAND_LIST[selectedBand - 1]) {
                    for (
                        let i = 1;
                        i <= TABS.vtx.VTXTABLE_BAND_LIST[selectedBand - 1].vtxtable_band_frequencies.length;
                        i++
                    ) {
                        const channelName =
                            TABS.vtx.VTXTABLE_BAND_LIST[selectedBand - 1].vtxtable_band_frequencies[i - 1];
                        if (channelName > 0) {
    ...
    function save_vtx() {
        ...
        function save_vtx_config() {
            MSP.send_message(
                ...
                mspHelper.crunch(MSPCodes.MSP_SET_VTX_CONFIG),
                false,
    ...
    function dump_html_to_msp() {
        ...
        for (let i = 1; i <= FC.VTX_CONFIG.vtx_table_powerlevels; i++) {
            TABS.vtx.VTXTABLE_POWERLEVEL_LIST[i - 1] = {};
            TABS.vtx.VTXTABLE_POWERLEVEL_LIST[i - 1].vtxtable_powerlevel_number = i;
            TABS.vtx.VTXTABLE_POWERLEVEL_LIST[i - 1].vtxtable_powerlevel_value = parseInt(
        ...
        for (let i = 1; i <= FC.VTX_CONFIG.vtx_table_bands; i++) {
            ...
            TABS.vtx.VTXTABLE_BAND_LIST[i - 1].vtxtable_band_is_factory_band = TABS.vtx.vtxTableFactoryBandsSupported
                ? $(`#vtx_table_band_factory_${i}`).prop("checked")
                : false;

            TABS.vtx.VTXTABLE_BAND_LIST[i - 1].vtxtable_band_frequencies = [];
            for (let j = 1; j <= FC.VTX_CONFIG.vtx_table_channels; j++) {
                TABS.vtx.VTXTABLE_BAND_LIST[i - 1].vtxtable_band_frequencies.push(
                    parseInt($(`#vtx_table_band_channel_${i}_${j}`).val()),
                );
            }
        }
    }
